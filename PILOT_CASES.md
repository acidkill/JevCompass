...
| P09 | Supplemental prompt, outside the 20-case core | CLI | `coding / python` | Implement normalize_whitespace in the synthetic Python project, then find and run its required validation. The copied fixture removes the README test command and adds a local CI workflow requiring `python -m unittest discover -s tests -v`. Blind quality, exact CI invocation, and standalone equivalent suite without `-v` are scored separately; old substring receipts cannot be rescored. |
