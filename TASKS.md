...
| VCR-02 | Make prompt advice relevant in vanilla sessions and name built-in tools as Codex exposes them | In progress | Substantive prompts can receive advice independent of unreliable permission-mode signals; trivial prompts skip; configured-only MCPs are excluded; built-in command recommendations use the actual Codex tool identifier; unit suite and fresh CLI delivery smoke pass. |
| VCR-03 | Make the local catalog and doctor useful on an uncustomized Codex install | Planned | Discover user-installed skills and configured integrations without exposing private inventory remotely; distinguish configured integrations from tools callable in this session; pipx install/upgrade and doctor behavior verified. |
