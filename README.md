...
To install from source instead, clone this repository and run `pipx install .` from the checkout. For a reproducible release, use `pipx install jevcompass==0.1.20`; check [releases](https://github.com/acidkill/JevCompass/releases) for the current verified version. Version 0.1.20 suppresses a generic shell-only package-docs suggestion seen in a 0.1.19 paired trial; a task-specific installed skill remains eligible. The latest source on `main` may contain unreleased changes.
