...
The advice context places locally validated candidate IDs immediately after its trace ID and before explanatory prose. Installed 0.1.21 sometimes emitted the full `exec_command`/`unittest` advisory while the agent's pre-tool report omitted the candidates. One source-built fictional-repository CLI smoke reported both IDs before first tool (9.24 ms). PR #101 passed hosted CI at 8369bb9, 276 Python 3.11 tests, graphify rebuild, wheel/sdist audit and isolated pipx wheel setup. Annotated v0.1.22 peels to 9720cd7; exact-tag GitHub asset SHA-256 wheel `c0492283467e4018f445a50999fd92ffc4b2e08993c24d3b48874e9b372de454` and sdist `9a23a2aa10359160b7e8f704939f8cd0a861b7ea2dcb41217bbbad605942a3f4` match local artifacts. Trusted Publishing run 36156580002 passed, and all 13 Python/catalog/skill wheel members matched the registry build by bytes. Fresh registry pipx 0.1.22 installation and two-hook doctor passed; an isolated CLI 0.155.1 session reported ID `0623c35b` and both candidate IDs before first tool, matching `testing/local` at 5.99 ms, exit 0 and an unchanged synthetic fixture. A subsequent maintainer-profile pipx upgrade to 0.1.22 preserved the `hooks.json` hash and doctor found two hooks. One native Desktop `worker` reported ID `9ceb0fdd` and `exec_command`/`jevcompass-focused-tests` before first tool; matching `SubagentStart/coding/local` took 54.64 ms. A later corrected P07 synthetic contract-writing pair on published 0.1.22 found a blind baseline quality win, but treatment reported `api-design/insufficient-candidates` and received no advice; this cannot establish an effect of JevCompass. This is a role delivery smoke, not a controlled effectiveness result; fresh Desktop prompt delivery, macOS runtime and broad paired benefit remain unverified.
...
Package documentation prompts no longer receive advice when the sole locally available candidate is the generic `exec_command` shell. The same low-signal rule already applies to other categories. A relevant installed packaging skill remains eligible, and remote Jev ranking for sufficiently specific candidate sets is unchanged. The change follows an installed 0.1.19 P05 blind README-quality tie with only a generic shell recommendation and no rated productive-action benefit; it is a noise reduction, not a demonstrated speed improvement. Two default nonblocking hooks, key handling, data minimization and required project checks are unchanged. The 273-test Python 3.11 suite, Graphify rebuild, wheel/sdist audit and isolated Python 3.11 pipx wheel setup passed locally. PR #96 passed hosted CI at c4bad69; annotated v0.1.20 peels to f2452f9. Exact-tag GitHub assets match local SHA-256 wheel `e96451950f238a7178992c51b5f40d3ce313e668bfad53bfb4a0c9ba3c5f2598` and sdist `bf2b8c3c913602b226108b01a0c95813cee1e25bcc278202148436d7d80b2eca`. [PyPI Trusted Publishing run 36151943706](https://github.com/acidkill/JevCompass/actions/runs/36151943706) passed; all 13 Python/catalog/skill wheel members match the registry build by bytes. A fresh registry pipx Python 3.11 install passed after index propagation, with two advisory hooks, no Jev PreToolUse gate, and doctor PASS. The maintainer's upgrade preserved the production hooks.json hash. A fresh isolated Codex CLI 0.155.1 P05 smoke then confirmed `package-docs/low-signal-skip` in 5.73 ms, no agent-visible advisory before its first tool, and a successful fictional CLI help check. This validates abstention only.

Install the published `jevcompass==0.1.20`. Fresh Desktop prompt delivery, macOS runtime and repeatable benefit remain unverified.

---

# JevCompass v0.1.19

This release avoids offering a globally installed `pytest` as the Python testing tool when a repository's local GitHub Actions commands unambiguously require `python -m unittest`. In that case the catalog offers a generic `unittest` candidate and still asks the agent to inspect the exact repository command. A pytest CI command remains eligible; mixed runner signals leave both as project decisions rather than assuming unittest. CI workflow content stays local; only generic candidate metadata reaches optional Jev ranking. The trigger was a published 0.1.18 remote choice for `testing/python` that selected pytest in this unittest-gated repository. This change improves candidate relevance, not a measured speed or quality gain. PR #87 passed hosted CI at a144463 and 272 local Python 3.11 tests. Annotated v0.1.19 peels to 3f74c74; exact-tree wheel/sdist audit and [PyPI Trusted Publishing run 36145304388](https://github.com/acidkill/JevCompass/actions/runs/36145304388) passed. A clean registry pipx installation initially hit a stale uv index cache, then passed with `UV_NO_CACHE=1`; two-hook setup, doctor and local unittest/no-pytest recommendation passed. The maintainer's pipx upgrade preserved the production hooks.json hash. A subsequent isolated CLI 0.155.1 smoke with installed 0.1.19 reported `unittest` advice ID `215744b8` before its first tool (local hook, 33.66 ms); the fictional required suite failed an existing assertion. This confirms local delivery only, not remote ranking or a measured benefit. A subsequent randomized installed 0.1.19 P01 pair with equal optional skills also delivered local advice, but both required unittest receipts were unknown and a quality correction happened after unblinding; it does not satisfy the efficacy gate (details in PILOT.md). A fresh repeat with corrected predeclared rubric found both anonymous helpers correct; after mapping, treatment had a recognized unittest exit 0 while baseline validation was unobserved. This one keyless local-advice comparison remains insufficient for a causal or broad-benefit claim. A separate 0.1.19 P05 documentation pair tied on blind README quality despite treatment's generic `exec_command` advice; its required fixture unittest failed, baseline test execution was unobserved, and productive-action timing stayed unscored.
