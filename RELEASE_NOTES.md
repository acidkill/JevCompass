# JevCompass v0.1.21 (candidate)

Shell-invoked commands such as `unittest` are now composed with the `exec_command` executor locally; Jev no longer receives a false either-or decision for that pair. Genuine alternatives among available tools or skills can still be ranked remotely, and the relevant command stays in the resulting advice. The clean-profile installed 0.1.20 synthetic request that exposed the issue took 321.15 ms and returned a local fallback; two new contract tests cover the corrected composition and skill-selection path, with 275 local Python 3.11 tests passing. Hosted CI, wheel/sdist checks, tagged publication, clean pipx installation and host-level delivery for this candidate remain pending. No broad speed or quality effect is claimed. Published 0.1.20 remains the verified release until those checks complete.
