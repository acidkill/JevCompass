...
TRIAGE_IDS = ("import_module_missing", "import_path_changed")
...
TREATMENT_TRIAGE = (
    "\n\nAfter the initial focused test command has completed with a failing "
    "exit status, run exactly "
    "python -m jevcompass triage --exit-code <observed-exit> --kind import "
    "--hypothesis import_module_missing --hypothesis import_path_changed --json. "
    "The CLI only recommends diagnostic steps; do not treat it as a test or "
    "diagnostic. Continue with your own read-only discriminator and minimal "
    "source fix, then run the exact focused and required suites from the task."
)
...
def _event_receipts(
    ...
    pending_triage: dict[str, tuple[int, bool]] = {}
    ...
    triage_after_initial_failure = False
    focused_count = 0
    full_count = 0
    ...
    for index, line in enumerate(lines):
        ...
        if event_type == "item.started" and event_id:
            ...
            # The first focused exit is not known until its completion event.
            # Re-evaluate the command using that exit when triage starts.
            if common._command_argv(item) and "jevcompass" in common._command_argv(item):
                triage_invocation_observed = True
                after_failure = first_focused_exit is not None and first_focused_exit != 0
                exact = _is_triage(item, first_focused_exit)
                accepted = after_failure and exact
                triage_after_initial_failure = triage_after_initial_failure or accepted
                triage_output_status = "captured" if accepted else "out_of_order_or_invalid_command"
                pending_triage[event_id] = (index, accepted)
            ...
            if event_id in pending_triage:
                _, accepted = pending_triage.pop(event_id)
                code = item.get("exit_code")
                triage_exit = code if isinstance(code, int) and not isinstance(code, bool) else None
                output = item.get("aggregated_output")
                if not isinstance(output, str):
                    output = item.get("output")
                if accepted and first_focused_exit is not None and triage_exit == 0:
                    triage_choice = _validated_triage(output, first_focused_exit)
                    triage_output_status = (
                        "valid_choice" if triage_choice["status"] != "unscored" else "invalid_output"
                    )
                elif accepted:
    ...
    return {
        ...
        "triage_after_initial_failure": triage_after_initial_failure,
...
def _arm_passed(
    ...
    # Codex events establish that the focused rerun follows the initial
    # failure, but do not prove that a source edit preceded that rerun.
    # Require an independently changed final artifact, without claiming that
    # this proves edit-before-test ordering.
    process_gate = (
    ...
    return (
        arm.get("triage_after_initial_failure") is True
        and arm.get("triage_cli_exit_code") == 0
        and arm.get("triage_output_status") == "valid_choice"
